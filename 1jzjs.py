...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "start_game":
        ...
        # Подсчет, сколько раз каждая клетка была безопасной
        safe_scores = [0] * 25
        for game in u["games"]:
            for i in range(25):
                if i not in game:
        ...
        # Выбираем 5 клеток с наибольшим количеством попаданий в безопасные
        top_safe = sorted(range(25), key=lambda i: safe_scores[i], reverse=True)[:5]
        safe_set = set(top_safe)

        # Создаем клавиатуру с 💎 и 💣
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("💎" if i in safe_set else "💣", callback_data="none") for i in range(r*5, r*5+5)]
            for r in range(5)
        ])
        await q.edit_message_text("Прогноз: 5 безопасных клеток 💎", reply_markup=kb)
        await q.message.reply_text("Новая сессия?", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Новый прогноз", callback_data="new_session")]]))reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Новый прогноз", callback_data="new_session")]]))
    elif d == "new_session":
